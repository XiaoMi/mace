...
#ifndef MACE_UTILS_TRANSPOSE_H_
#define MACE_UTILS_TRANSPOSE_H_
...
constexpr int64_t kCopyBlockSize = 1024 * 1024 * 100;  // 100MB
...
inline void CopyDataBetweenSameType(mace::utils::ThreadPool *thread_pool,
                                    const void *src, void *dst, int64_t size) {
  if (size < kCopyBlockSize || thread_pool == nullptr) {
    memcpy(dst, src, size);
  } else {
    thread_pool->Compute1D(
        [=](int64_t start, int64_t end, int64_t step) {
          MACE_UNUSED(step);
          memcpy(static_cast<char *>(dst) + start,
                 static_cast<const char *>(src) + start, end - start + 1);
        },
...
template<>
inline void CopyDataBetweenDiffType<BFloat16, half>(
