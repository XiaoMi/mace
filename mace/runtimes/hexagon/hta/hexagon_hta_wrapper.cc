...
#include "mace/runtimes/hexagon/hta/hexagon_hta_ops.h"
...
HexagonHTAWrapper::HexagonHTAWrapper(Runtime *runtime,
                                     std::shared_ptr<Rpcmem> rpcmem)
    : runtime_(runtime),
      rpcmem_(rpcmem),
#ifdef MACE_ENABLE_OPENCL
    transformer_(make_unique<HexagonHTATranformer<RT_OPENCL>>()) {
#else
    transformer_(make_unique<HexagonHTATranformer<RT_CPU>>()) {
#endif  // MACE_ENABLE_OPENCL

  transformer_->Init(runtime);
}
...
bool HexagonHTAWrapper::SetupGraph(const NetDef &net_def,
  ...
  for (int index = 0; index < num_inputs_; ++index) {
    ...
    auto quantized_tensor = make_unique<Tensor>(
        runtime_, DT_UINT8, runtime_->GetBaseMemoryType());
    quantized_tensor->Resize(input_shape);
    auto hta_tensor = make_unique<Tensor>(
        runtime_, DT_UINT8, runtime_->GetBaseMemoryType());
    ...
    input_tensordef.fd = rpcmem_->ToFd(hta_tensor->mutable_data<void>());
  ...
  for (int index = 0; index < num_outputs_; ++index) {
    ...
    auto quantized_tensor = make_unique<Tensor>(
        runtime_, DT_UINT8, runtime_->GetBaseMemoryType());
    quantized_tensor->Resize(output_shape);
    auto hta_tensor = make_unique<Tensor>(
        runtime_, DT_UINT8, runtime_->GetBaseMemoryType());
    ...
    output_tensordef.fd = rpcmem_->ToFd(hta_tensor->mutable_data<void>());
