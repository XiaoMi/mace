...
#include "mace/runtimes/hexagon/hta/hexagon_hta_transformer.h"
...
template<>
class QuantizeTransformer<RuntimeType::RT_CPU> : public BaseTransformer {
 public:
  void Init(Runtime *runtime) override {
    BaseTransformer::Init(runtime);
    quantize_util_.Init(&runtime->thread_pool());
  }
...
#ifdef MACE_ENABLE_OPENCL
template <>
class QuantizeTransformer<RuntimeType::RT_OPENCL> : public BaseTransformer {
 public:
  MaceStatus Compute(const Tensor *input, Tensor *output) override {
...
template<>
class DequantizeTransformer<RuntimeType::RT_CPU> : public BaseTransformer {
 public:
  void Init(Runtime *runtime) override {
    BaseTransformer::Init(runtime);
    quantize_util_.Init(&runtime->thread_pool());
...
#ifdef MACE_ENABLE_OPENCL
template <>
class DequantizeTransformer<RuntimeType::RT_OPENCL> : public BaseTransformer {
 public:
  MaceStatus Compute(const Tensor *input, Tensor *output) override {
...
template<>
class NHWCToNCHW32Transformer<RuntimeType::RT_CPU> : public BaseTransformer {
 public:
  MaceStatus Compute(const Tensor *input, Tensor *output) override {
    ...
    runtime_->thread_pool().Compute2D(
...
#ifdef MACE_ENABLE_OPENCL
template <>
class NHWCToNCHW32Transformer<RuntimeType::RT_OPENCL> : public BaseTransformer {
 public:
  MaceStatus Compute(const Tensor *input, Tensor *output) override {
    ...
    MACE_RETURN_IF_ERROR(TuningOrRun3DKernel(executor, kernel_,
...
template<>
class NHWCToD32Transformer<RuntimeType::RT_CPU> : public BaseTransformer {
 public:
  MaceStatus Compute(const Tensor *input, Tensor *output) override {
...
#ifdef MACE_ENABLE_OPENCL
template <>
class NHWCToD32Transformer<RuntimeType::RT_OPENCL> : public BaseTransformer {
 public:
  MaceStatus Compute(const Tensor *input, Tensor *output) override {
    ...
    std::vector<uint32_t> lws = {4, 4, 4, 0};
    MACE_RETURN_IF_ERROR(TuningOrRun3DKernel(
        executor, kernel_, tuning_key, gws, lws, nullptr));
    return MaceStatus::MACE_SUCCESS;
  }
...
template<>
class D32ToNHWCTransformer<RuntimeType::RT_CPU> : public BaseTransformer {
 public:
  MaceStatus Compute(const Tensor *input, Tensor *output) override {
...
#ifdef MACE_ENABLE_OPENCL
template <>
class D32ToNHWCTransformer<RuntimeType::RT_OPENCL> : public BaseTransformer {
 public:
  MaceStatus Compute(const Tensor *input, Tensor *output) override {
    MACE_LATENCY_LOGGER(1, "D32ToNHWCTransformer on GPU");
...
template<RuntimeType D>
void HexagonHTATranformer<D>::Init(Runtime *runtime) {
  runtime_ = runtime;
  quantizer_ = make_unique<QuantizeTransformer<D>>();
  quantizer_->Init(runtime);
  dequantizer_ = make_unique<DequantizeTransformer<D>>();
  dequantizer_->Init(runtime);
}
...
MaceStatus HexagonHTATranformer<D>::SetInputTransformer(
  ...
  input_transformers_.back()->Init(runtime_);
  return MaceStatus::MACE_SUCCESS;
}
...
MaceStatus HexagonHTATranformer<D>::SetOutputTransformer(
  ...
  output_transformers_.back()->Init(runtime_);
  return MaceStatus::MACE_SUCCESS;
}
...
template void HexagonHTATranformer<RT_CPU>::Init(Runtime *runtime);
template MaceStatus HexagonHTATranformer<RT_CPU>::Quantize(const Tensor *input,
                                                           Tensor *output);
template MaceStatus HexagonHTATranformer<RT_CPU>::Dequantize(
    const Tensor *input, Tensor *output);
template MaceStatus HexagonHTATranformer<RT_CPU>::SetInputTransformer(
    hexagon_hta_hw_layout format);
template MaceStatus HexagonHTATranformer<RT_CPU>::SetOutputTransformer(
    hexagon_hta_hw_layout format);
template MaceStatus HexagonHTATranformer<RT_CPU>::TransformInput(
    const Tensor *input, Tensor *output, int index);
template MaceStatus HexagonHTATranformer<RT_CPU>::TransformOutput(
    const Tensor *input, Tensor *output, int index);

#ifdef MACE_ENABLE_OPENCL
template void HexagonHTATranformer<RT_OPENCL>::Init(Runtime *runtime);
template MaceStatus HexagonHTATranformer<RT_OPENCL>::Quantize(
    const Tensor *input, Tensor *output);
template MaceStatus HexagonHTATranformer<RT_OPENCL>::Dequantize(
    const Tensor *input, Tensor *output);
template MaceStatus HexagonHTATranformer<RT_OPENCL>::SetInputTransformer(
    hexagon_hta_hw_layout format);
template MaceStatus HexagonHTATranformer<RT_OPENCL>::SetOutputTransformer(
    hexagon_hta_hw_layout format);
template MaceStatus HexagonHTATranformer<RT_OPENCL>::TransformInput(
    const Tensor *input, Tensor *output, int index);
template MaceStatus HexagonHTATranformer<RT_OPENCL>::TransformOutput(
    const Tensor *input, Tensor *output, int index);
#endif  // MACE_ENABLE_OPENCL
}  // namespace mace
