...
template<>
class QuantizeOp<RuntimeType::RT_CPU, uint8_t> : public Operation {
 public:
  explicit QuantizeOp(OpConstructContext *context)
      : Operation(context),
        ...
        find_range_every_time_(
            Operation::GetOptionalArg<bool>("find_range_every_time", false)),
        quantize_util_(&context->runtime()->thread_pool()) {}
...
template<typename T>
class DequantizeOp<RuntimeType::RT_CPU, T> : public Operation {
 public:
